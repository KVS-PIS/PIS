...
<?php 
	...
	if($name_array['status']){
		//$final_name1  = 'Name of the'.' '.$name_array['label'].'- '.$name_array['name'] ;
		$final_name1  = ''  ; // second axis y title 
		$final_name2  = 'No. of Sanctioned / In-Positions'.' '.$name_array['label'].'- '.$name_array['name'] ;
	}else{
		$region_id = $this->session->userdata['region_id'];
		$session_code = getCodeByRegionId($region_id);
        ...
        <div class="container-fluid">
            ...
            <ol class="breadcrumb">
                <li class="breadcrumb-item">
                    <a href="#">REGIONAL OFFICE DASHBOARD</a>
                </li>
                ...
                <div class="col-xl-3 col-sm-6 mb-3">
                    <div class="card text-white bg-success o-hidden h-100">
                        <div class="card-body">
                            ...
                            <div class="mr-5">Total Transfer Request  </div>
                        </div>
                        <a class="card-footer text-white clearfix small z-1" href="#">
...
// Add data
chart.data = [
  <?php if(isset($school_by_region) && !empty($school_by_region)){
	  $DH=100*COUNT($VAC_VS_INPOS);
 foreach($school_by_region as $school){
	 $name = get_scholl_name_by_ccde_and_type($school['present_kvcode'],$school['type']);
	 if(!empty($name)){
	?>
{
  "country": "<?php echo ($name)? $name : '' ?>",
  "visits": <?php echo ($school['total'])? $school['total'] : '' ?>,
...
title.text = "No. of Employee Registered By KV";
title.fontSize = 20;
...
chart.data = [
        <?php if(isset($VAC_VS_INPOS) && !empty($VAC_VS_INPOS)){
            ...
            if(!empty($V['NAME'])){
                $region_id = $this->session->userdata['region_id'];
                if($region_id){
                    $V['DataUrl']=($V['KVC'])? base_url().'admin/dashboard/index/'.$V['KVC'].'/3' : '';
                }else{
