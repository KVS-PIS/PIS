<?php 
	...
	$final_name2 = "No. of Sanctioned / In-Positions" ;
	$final_name='';	
	} 
?>
<!DOCTYPE html>
...
// Add data
chart.data = [
        <?php if(isset($EMP_REGD) && !empty($EMP_REGD)){
            $DH=100*COUNT($VAC_VS_INPOS);
            foreach($EMP_REGD as $R){
            if(!empty($R['HQ_ZT_RO'])){
	?>
            {
              "country": "<?php echo ($R['HQ_ZT_RO'])? $R['HQ_ZT_RO'] : '' ?>",
              "visits": <?php echo ($R['TOT'])? $R['TOT'] : '' ?>,
